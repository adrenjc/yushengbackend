...
async function productionCleanup() {
  ...
  try {
    ...
    console.log("\n📊 清理结果:")
    console.log(
      `   ✓ 匹配记录: ${deletedRecords.deletedCount}/${stats.matchingRecords}`
    )
    console.log(
      `   ✓ 匹配任务: ${deletedTasks.deletedCount}/${stats.matchingTasks}`
    )
    console.log(
      `   ✓ 商品数据: ${deletedProducts.deletedCount}/${stats.products}`
    )
    console.log(
      `   ✓ 商品模板: ${deletedTemplates.deletedCount}/${stats.templates}`
    )
...
async function main() {
  ...
  // 环境确认
  const env = await askConfirmation(
    "1. 当前环境 (输入 'production' 确认这是生产环境): "
  )
  ...
  // 备份确认
  const backup = await askConfirmation(
    "2. 数据备份 (输入 'backed-up' 确认已完成数据备份): "
  )
  ...
  // 最终确认
  const code = await askConfirmation(
    `3. 最终确认 (输入确认码 '${CONFIRMATION_CODE}' 执行清理): `
  )
